#!/usr/bin/env node
// Erzeugt supabase/seed_bibel_v4.sql aus data/questions_bibel_v4.json
// (DREISPRACHIG: de/en/ru, index-gleich). Spielt NUR neue Fragen ein
// (status='approved', stabile IDs v4b-NNNN, on conflict do nothing).
// Kategorie: jede Frage traegt den Tag "bibel" (kind=type) -> eigene Rubrik.
// Optionen werden je Frage deterministisch gemischt (richtige Antwort nicht
// immer Position 0).
//
// Aufruf:  node scripts/build_import_v4.mjs
// Danach:
//   psql "<conn>" -v ON_ERROR_STOP=1 -f supabase/seed_bibel_v4.sql
//   ./scripts/refresh-assets.sh     # Assets (de/en/ru) aus der DB neu bauen
//   git commit + Deploy
...
const SRC = resolve(ROOT, 'data/questions_bibel_v4.json')
const OUT = resolve(ROOT, 'supabase/seed_bibel_v4.sql')
const CHUNK = 800
...
// Lokalisierte Kategorienamen (nur relevant fuer die neue "bibel"-Rubrik;
// Anzeige laeuft ohnehin ueber trCategory in der App).
const CAT_NAMES = {
  bibel: { en: 'Bible Facts', de: 'Bibel', ru: 'О Библии' },
}
...
items.forEach((it, n) => {
  const id = 'v4b-' + String(n + 1).padStart(4, '0')
  const qid = uuid('q:' + id)
  const difficulty = LEVEL[it.level] || 2
  questions.push(`(${q(qid)}, ${difficulty}, 'approved', 'bibel-v4')`)

  // explanation = Bibelstelle (optional); leer -> null
  const refDe = (it.ref_de || '').trim()
  const refEn = (it.ref_en || '').trim()
  const refRu = (it.ref_ru || '').trim()
  qTr.push(`(${q(qid)}, 'de', ${q(it.q_de.trim())}, ${refDe ? q(refDe) : 'null'})`)
  qTr.push(`(${q(qid)}, 'en', ${q(it.q_en.trim())}, ${refEn ? q(refEn) : 'null'})`)
  qTr.push(`(${q(qid)}, 'ru', ${q(it.q_ru.trim())}, ${refRu ? q(refRu) : 'null'})`)

  // Optionen gemischt einsetzen, Tripel DE/EN/RU zusammenhalten.
  ...
  order.forEach((src, pos) => {
    ...
    optTr.push(`(${q(oid)}, 'ru', ${q(String(it.options_ru[src]).trim())})`)
  })
...
for (const [slug, { tag, kind }] of tagMap) {
  ...
  if (names) {
    for (const lang of ['en', 'de', 'ru']) {
      catTr.push(`(${q(cid)}, ${q(lang)}, ${q(names[lang])})`)
    }
  } else {
...
const lines = ['-- AUTO-GENERIERT (build_import_v4.mjs) – dreisprachige "Bibel"-Fakten, NUR neue Zeilen', 'begin;']
...
console.log(`✔ supabase/seed_bibel_v4.sql: ${items.length} Fragen, ${tagMap.size} Kategorien, ${questions.length} Frage-Zeilen, ${opts.length} Optionen`)
