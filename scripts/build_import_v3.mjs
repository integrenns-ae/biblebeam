#!/usr/bin/env node
// Erzeugt supabase/seed_hard_v3.sql aus data/questions_hard_v3.json
// (zweisprachige Phrasen-Fragen, DE+EN index-gleich). Spielt NUR neue Fragen ein
// (status='approved', stabile IDs v3h-NNNN, on conflict do nothing) – die
...
//   psql "<conn>" -v ON_ERROR_STOP=1 -f supabase/seed_hard_v3.sql
...
const SRC = resolve(ROOT, 'data/questions_hard_v3.json')
const OUT = resolve(ROOT, 'supabase/seed_hard_v3.sql')
const CHUNK = 800
...
items.forEach((it, n) => {
  const id = 'v3h-' + String(n + 1).padStart(4, '0')
  const qid = uuid('q:' + id)
...
console.log(`✔ supabase/seed_hard_v3.sql: ${items.length} Fragen, ${tagMap.size} Kategorien, ${questions.length} Frage-Zeilen, ${opts.length} Optionen`)
